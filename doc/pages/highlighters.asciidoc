...
with a pipe sign) of *colorize*, *move*, *wrap* or *replace*
(default: *colorize*):

----------------------------------------------------------------------
add-highlighter window/<name> group -passes colorize|move|wrap|replace
----------------------------------------------------------------------
...
Like the group highlighter, ref accepts a *-passes* flag set selecting the
kinds of highlighters that are forwarded (default: *colorize*). A shared
highlighter running in another pass, such as *replace-ranges*, needs the
matching pass to be listed explicitly:

---------------------------------------------------------
add-highlighter shared/<name> replace-ranges <option_name>
add-highlighter window/ ref -passes replace <name>
---------------------------------------------------------
