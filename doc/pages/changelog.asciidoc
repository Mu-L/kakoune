...
* A local scope is now active when evaluating a command line

* The `-passes` switch of the `group` and `ref` highlighters now accepts
  `replace`, allowing `replace-ranges` highlighters to be put in groups
  and referenced from the shared scope
